...
# The rename to agent-config changed every name install uses to recognize its own
...
legacyize() {  # legacyize <fake home> -- turn a current install into a 0.3.x one
  ...
  mv "$h/.claude/.agent-config-origins" "$h/.claude/.agent-config-origins"
  [[ -f "$s.before-agent-config" ]] && mv "$s.before-agent-config" "$s.before-agent-config"
  [[ -f "$s.agent-config-deny.json" ]] && mv "$s.agent-config-deny.json" "$s.agent-config-deny.json"
  sed -i.bak 's/agent-config-hook-v1/agent-config-hook-v1/g' "$s"
  sed -i.bak -e 's/agent-config-hook-v1/agent-config-hook-v1/g' \
             -e 's/guardrails from agent-config/guardrails from agent-config/' "$c"
  rm -f "$s.bak" "$c.bak"
  return 0
}
...
out="$(HOME="$H" AGENT_CONFIG_NONINTERACTIVE=1 bash "$S/repo/install.sh" full 2>&1)"; code=$?
...
HOME="$H" bash "$OLD/install.sh" full >/dev/null 2>&1
legacyize "$H"
HOME="$H" AGENT_CONFIG_NONINTERACTIVE=1 bash "$S/repo/install.sh" full >/dev/null 2>&1
out="$(HOME="$H" AGENT_CONFIG_NONINTERACTIVE=1 bash "$S/repo/install.sh" full 2>&1)"; code=$?
...
# PREVIOUS agent-config version sitting next to the new one, and every release would
# have added another forever. Found on a real machine after shipping 0.4.1:
# 0.4.0 was still there.
H="$S/h84"; mkdir -p "$H"
PREV="$H/.local/share/agent-config/0.0.1"
...
H="$S/h84b"; mkdir -p "$H"
CUR="$H/.local/share/agent-config/9.9.9"
mkdir -p "$(dirname "$CUR")"; cp -R "$S/repo" "$CUR"
HOME="$H" AGENT_CONFIG_NONINTERACTIVE=1 bash "$CUR/install.sh" full >/dev/null 2>&1
